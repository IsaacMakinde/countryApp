...
class CountryService {
  ...
  async getCountriesByName(name: string): Promise<IServiceResponse<Country[]>> {
    try {
      const response = await this.httpClient.get(`http://localhost:5001/countries/name/${name}`);
      if (!response.ok) {
  ...
  async getCountriesByLanguage(language: string): Promise<IServiceResponse<Country[]>> {
    try {
